...
def write_transformation_to_csv_file(bag_file, target_frame, csv_file_name):
    ...
    init = True
    tf_counter = 0
    tf_frequency_estimated = 0.0
    tf_interval_estimated = 0.0
    tf_previous_timestamp = 0.0
    ...
    if tf_counter > 3:
        tf_frequency_estimated = (
            tf_counter / (end_time_tf_message - start_time_tf_message).to_sec()
        )
        print(
            "Final estimate of tf topic frequency: ",
            "{0:.2f}".format(tf_frequency_estimated),
            "Hz",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tf_target_frame",
        default="/atracsys/Camera_hand/measured_cp",
        help="Name of tf target frame.",
    )
    parser.add_argument("--csv_output_file", help="Path to output csv file")

    args = parser.parse_args()
    out_dir = args.bag[:-4] + ".csv"  # args.csv_output_file
    print("tf_to_csv.py: export tf to csv from bag: ", args.bag, "...")

    write_transformation_to_csv_file(args.bag, args.tf_target_frame, out_dir)
