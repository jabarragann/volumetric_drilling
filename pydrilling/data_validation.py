...
def verify_xyz(depth, K):
    ...
    assert np.all(
        np.isclose(xyz, depth, rtol=0.01, atol=1e-3)
...
def verify_sphere(depth, K, RT, pose_cam, pose_primitive, time_stamps):
    ...
    for i in range(len(depth_output)):
        ...
        error_str = "Anal_z: " + toStr(z_act) + " Depth_z: " + toStr(z_mea) + " Diff: "
        if abs(err) < 0.01:
    ...
    # print(depth_output)
    assert np.all(np.isclose(z, depth_output, rtol=0.01)), "fail, largest error %f" % (
        np.max(np.abs(z - depth_output))
    )
